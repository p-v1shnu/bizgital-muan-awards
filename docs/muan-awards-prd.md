...
| | |
|---|---|
| **เวอร์ชัน** | 1.1 (แยกสวิตช์ฟอร์มออกจาก phase · เฟสเหลือ 4) |
...
5. เมนู "ปีปัจจุบัน", แถบสถานะหน้าแรก และ `/awards/latest` ดึงจาก **ปีที่ `phase ≠ DRAFT` ล่าสุด** อัตโนมัติเสมอ — ไม่ hardcode ปีในโค้ด
...
| 4 | จัดการปี | `/admin/editions` + `/admin/editions/[id]` | สร้าง/แก้ไขปี, **ปุ่มเปลี่ยนเฟส + สวิตช์เปิด/ปิดฟอร์มส่งรายชื่อ (แยกกัน)**, จัดการภายในปีเป็น tab: ข้อมูลงาน / สาขา / นอมินี / กรรมการ / สปอนเซอร์ |
...
**รองรับด้วยกติกาข้อ 4.3 อยู่แล้ว ไม่ต้องมี flag พิเศษ:** สร้างปี → ตอนยังเป็น `DRAFT` ตั้งเฟสเป็น `WINNERS_ANNOUNCED` ได้เลย → ใส่สาขา/นอมินี/ผู้ชนะ → เผยแพร่ ส่วน `submissionsOpen` ปล่อยเป็น `false` ไว้ ฟอร์มจึงไม่มีวันเปิดให้ปีที่จบไปแล้ว
...
3. `phase` เดินหน้าอย่างเดียวหลังพ้น `DRAFT` · ตั้งเฟสเริ่มต้นได้อิสระตอนยังเป็น `DRAFT` · `submissionsOpen` เปิด/ปิดได้อิสระตลอดเวลา และเปิดได้ครั้งละ 1 ปี (ข้อ 4.3)
