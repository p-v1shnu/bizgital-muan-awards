...
| **ออกจากระบบไม่ได้ยกเลิกอะไรเลย** | logout แค่ลบ cookie — refresh token ข้างในยังใช้ได้อีก **7 วัน** (ทดสอบแล้ว: ใช้ตัวเดิมซ้ำได้ 200) · เปลี่ยนรหัสผ่านก็ไม่เตะเครื่องอื่นออก | ✅ เพิ่ม `tokenVersion` — logout / เปลี่ยนรหัสผ่าน = **ตัดทุก session ทันที** ทั้ง refresh และ access token ที่ยังไม่หมดอายุ (แลกกับ: ออกจากระบบเครื่องเดียว = ออกทุกเครื่อง ซึ่งเหมาะกับทีมเล็ก) |
...
**ตรวจแล้ว:** e2e หลังบ้าน **60 ข้อ** (เพิ่ม 5: ล็อกเอาต์ตัด session, เปลี่ยนรหัสผ่านตัด session, ล็อกบัญชีหลังเดาผิด, IP อื่นยังเข้าได้, payload ใหญ่ได้ 413) · ขับเบราว์เซอร์ **37 ข้อ** — ผ่านหมด
